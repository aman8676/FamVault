...
export const FamilyCard = ({ family, onClick, isOwner }) => {
      ...
      >
        ...
        {isOwner && (
          <button
            ...
            className="mt-4 w-full py-2 rounded-xl text-xs font-semibold text-indigo-500 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors flex items-center justify-center gap-2 border border-indigo-100 dark:border-indigo-500/20"
          >
...
// Reset PIN Modal (only for owner/admin)

const ResetPinModal = ({ family, onClose }) => {
  const [newPin, setNewPin] = useState("");
  const [confirmPin, setConfirmPin] = useState("");
  const [resetting, setResetting] = useState(false);
  const [error, setResetError] = useState("");
  ...
  const handleReset = async (e) => {
    ...
    if (newPin.length < 4 || newPin.length > 6) {
      return setResetError("PIN must be 4–6 digits");
    }
    if (newPin !== confirmPin) {
      return setResetError("PINs do not match");
    }

    setResetting(true);
    try {
      const { data } = await api.put(
        `/families/${family._id}/reset-pin`,
        { newPin },
      );

      if (data.success) {
        setSuccess(
          `PIN reset! ${data.membersNotified} member(s) notified via email.`,
        );
        setTimeout(() => {
          onClose();
        }, 2000);
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div className="bg-white dark:bg-[#1a1a2e] rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 dark:border-white/[0.08] overflow-hidden">
        {/* Top accent bar */}
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleReset} className="space-y-3">
            {/* Error */}
            {error && (
              <p className="text-xs text-red-500 dark:text-red-400 text-center font-semibold bg-red-50 dark:bg-red-500/10 py-2 rounded-lg">
                {error}
              </p>
            )}
            ...
            {/* Success */}
            {success && (
              <p className="text-xs text-green-600 dark:text-green-400 text-center font-semibold bg-green-50 dark:bg-green-500/10 py-2 rounded-lg">
                ✓ {success}
              </p>
            )}
            ...
            <p className="text-[11px] text-gray-400 dark:text-gray-500 text-center">
              All family members will be notified via email 📧
            </p>
            ...
            <div className="flex gap-2.5 pt-1">
              ...
              <button
                type="submit"
                disabled={
                  resetting || newPin.length < 4 || confirmPin.length < 4
                }
      ...
      <div className="bg-white dark:bg-white/[0.06] rounded-2xl border border-gray-100 dark:border-white/[0.08] shadow-sm p-5 hover:shadow-md transition-shadow">
        ...
        {!showPin ? (
          <div className="space-y-2.5">
            ...
            {/* Reset PIN button — only for owner */}
            {isOwner && (
              <button
                onClick={() => setShowResetModal(true)}
                className="w-full py-2.5 rounded-xl text-xs font-semibold text-indigo-500 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors flex items-center justify-center gap-2 border border-indigo-100 dark:border-indigo-500/20"
              >
                <KeyRound className="w-3.5 h-3.5" />
                Reset PIN
              </button>
          ...
          <form onSubmit={handleVerify} className="space-y-3">
            ...
            {isOwner && (
              <p
                ...
                className="text-[11px] text-indigo-400 dark:text-indigo-400 text-center cursor-pointer hover:underline"
              >
                Forgot PIN? Reset it
              </p>
            )}
          ...
          {families.memberFamilies?.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.15em] mb-4 flex items-center gap-2.5">
                ...
                Member of ({families.memberFamilies.length})
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
